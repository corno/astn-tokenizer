...
function createRangeFromLocations(start: tokenLevel.Location, end: tokenLevel.Location): tokenLevel.Range {
    return {
        start: start,
        length: end.position - start.position,
        size: ((): tokenLevel.RangeSize => {
...
function createRangeFromSingleLocation(location: tokenLevel.Location): tokenLevel.Range {
    return {
        start: location,
        length: 0,
...
type BlockCommentContext = {
    locationOfFoundAsterisk: null | tokenLevel.Location
}
...
type FoundNewlineCharacter = {
    ...
    | ["carriage return", {}]
    | ["line feed", {}]
    startLocation: tokenLevel.Location
}
                        ...
                        (nextChar) => {
                            ...
                            if ($.slashed) {
                                ...
                                if ($.unicode.charactersLeft === 0) {
                                    const textNode = pr.StringFromCharCode(pr.parseInt($.unicode.foundCharacters, 16))
                                    $.unicode = null
                                    return {
                                        startSnippet: false,
                                        consumeCharacter: true,
